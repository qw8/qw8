...
+ 如果是第一次使用，需要先配置小程序ide的相关路径（比如微信开发者工具安装路径），才能运行成功
...
[文档](https://uniapp.dcloud.io/component/image?id=image)
...
```js
<view v-for="(item,i) in arr" :key="i">名字：\\\{\\\{item.nam\\\}\\\}e}}---年龄：\\\{\\\{item.ag\\\}\\\}e}}</view>
```
...
[链接](https://ask.dcloud.net.cn/article/35021)