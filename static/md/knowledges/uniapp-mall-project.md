  ...
  <view class="hot_goods">
    ...
    <view class="goods_list">
      <view class="goods_item" v-for="item in goods" :key="item.id">
        ...
        <view class="price">
          <text>\\\{\\\{item.sell_pric\\\}\\\}e}}</text>
          <text>\\\{\\\{item.market_pric\\\}\\\}e}}</text>
        </view>
        <view class="name">\\\{\\\{item.titl\\\}\\\}e}}</view>
      </view>
    </view>
  </view>
  ```
...
<template>
	<view>
		<view class="news_item" v-for="item in data" :key="item.id">
			<image :src="item.img_url"></image>
			<view class="content">
				<view class="tit">\\\{\\\{item.titl\\\}\\\}e}}</view>
				<view class="info">
					<text>发表时间：\\\{\\\{item.add_time | formatDat\\\}\\\}e}}</text>
					<text>浏览：\\\{\\\{item.click+12\\\}\\\}3}}次</text>
				</view>
			</view>
		</view>
	</view>
...
<view class="info">
  <text>发表时间：\\\{\\\{newsDetail.add_time | formatDat\\\}\\\}e}}</text>
<text>浏览：\\\{\\\{newsDetail.clic\\\}\\\}k}}</text>
</view>
<view class="content">
