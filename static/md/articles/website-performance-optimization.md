...
对于网站的性能，在行业内有很多既定的指标，但就以前端er而言，我们应该更加关注以下指标：白屏时间、首屏时间、整页时间、DNS时间、CPU占用率。而我之前自己搭建的一个网站（网址：jerryonlyzrj.com/resume/ ，近日因域名备案无法打开，几日后即恢复正常），完全没做性能优化时，首屏时间是12.67s，最后经过多方面优化，终于将其降低至1.06s，并且还未配置CDN加速。其中过程我踩了很多坑，也翻了许多专业书籍，最后决定将这几日的努力整理成文，帮助前端爱好者们少走弯路。文章更新可能之后不会实时同步在论坛上，欢迎大家关注我的Github，我会把最新的文章更新在对应的项目里，让我们一起在代码的海洋里策马奔腾：github.com/jerryOnlyZR… 。
...
在开始介绍网络传输性能优化这项工作之前，我们需要了解浏览器处理用户请求的过程，那么就必须奉上这幅神图了：![img](https://mmbiz.qpic.cn/mmbiz/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguHBrvsU0TdYS5T93mHxufLWN3ds4NocUmqdadk4libcjtDpL3ibibE1f1g/640?wx_fmt=other&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
我们都知道，浏览器在向服务器发起请求前，会先查询本地是否有相同的文件，如果有，就会直接拉取本地缓存，这和我们在后台部属的Redis和Memcache类似，都是起到了中间缓冲的作用，我们先看看浏览器处理缓存的策略：![img](https://mmbiz.qpic.cn/mmbiz/udZl15qqib0Op8L2Y8lPjG250iaAE9Elgu92ibS0iaj3FmM3XaZkxIrViasxHPzlIdCtBnz8MpC4b85tLBvSN7mLgPw/640?wx_fmt=other&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
（这里需要特别注意，在我们进行缓存调试时，需要去除network面板顶部的`Disable cache`勾选项，否则浏览器将始终不会从缓存中拉取数据）![img](https://mmbiz.qpic.cn/mmbiz/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguDLJAcqR9TQTUJNWdlsCSqBj5V5sicica80EcX0pCJRias8To64ZELKxsA/640?wx_fmt=other&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
```
    etag on;   //开启etag验证    expires 7d;    //设置缓存过期时间为7天
```

打开我们的网站，在chrome devtools的network面板中观察我们的请求资源，如果在响应头部看见Etag和Expires字段，就说明我们的缓存配置成功了。

**![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9Elgux77Zibpwu0GBZG9Olub3S2wp63WZ3lVGyLlG37NJDUiaWMDyljYsAiacQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)**
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9Elgu4FeMUabV5Hr569nZC63sTDIlHkz9RYvebObroRyicia0q8tib209HFaqg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
new HtmlWebpackPlugin(\\{
  template: __dirname + '/views/index.html', // new 一个这个插件的实例，并传入相关的参数         filename: '../index.html',
  minify: \\{
  removeComments: true,
...
我们在使用`html-webpack-plugin` 自动化注入JS、CSS打包HTML文件时，很少会为其添加配置项，这里我给出样例，大家直接复制就行。据悉，在Webpack5中，`html-webpack-plugin` 的功能会像 `common-chunk-plugin` 那样，被集成到webpack内部，这样我们就不需要再install额外的插件了。PS：这里有一个技巧，在我们书写HTML元素的`src` 或 `href` 属性时，可以省略协议部分，这样也能简单起到节省资源的目的。（虽然其目的本身是为了统一站内的所有协议）
...
use: [
_mode === 'development' ? 'style-loader' : MiniCssExtractPlugin.loader, \\{                       loader: 'css-loader',
options: \\{
importLoaders: 1
}}, 
...
⑤将webpack开发环境修改为生产环境：在使用webpack打包项目时，它常常会引入一些调试代码，以作相关调试，我们在上线时不需要这部分内容，通过配置剔除：   devtool: 'false'
...
雪碧图的概念大家一定在生活中经常听见，其实雪碧图是减小请求数的显著运用。而且很奇妙的是，多张图片聘在一块后，总体积会比之前所有图片的体积之和小（你可以亲自试试）。这里给大家推荐一个自动化生成雪碧图的工具：www.toptal.com/developers/…  （图片来自官网首页）

![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9ElgupbsaY5fWDTgIkLBpibXmlrhNohZ0FescYibwWFOTVhNBrAVJyXsyMHkA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_png/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguOiaojMokz9zfkibsLu4XpLRBJrW8yUt7SxKbd82VhTnCIxicAJWM2yDAg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

 然后，我们需要插件去读取这个文件夹内的所有图片资源文件，以文件夹名称为图片名称生成一张雪碧图到指定位置，并且输出能够正确使用这些雪碧图的CSS文件。

![img](https://mmbiz.qpic.cn/mmbiz_png/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguvV4qicDk1dcDZpStr6fYSeYhbnA9Q6mj5aicP6844VqTV3kibSric40nVQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

![img](https://mmbiz.qpic.cn/mmbiz_png/udZl15qqib0Op8L2Y8lPjG250iaAE9Elgu3ib3skQWZ1XP8cawjfE8aynxlpZLMxR2dHC6Um1qicic2UjWBe3dExTVA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

如今，`webpack-spritesmith`这款插件能实现我们想要的一切，先奉上配置内容：（具体可参照`webpack-spritesmith`官方文档：www.npmjs.com/package/web）

![img](https://mmbiz.qpic.cn/mmbiz_png/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguuDAtA97MYCCShOcgYQeEamoOy5kOVTUkj7Tw8Jn6mbcyBv23Ch9Nzg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

执行webpack之后，就会在开发目录里生成上面两张图的结果，我们可以看看`common.css`里面的内容：

![img](https://mmbiz.qpic.cn/mmbiz_png/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguIrBibL7niaPjWIpKoNicjmKEt6CPO2AYnwrfdQDwP2EBElYwOkCaxIroA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9Elgu3wTtatZsJQDrICfJ1b8aAXk1gacwMRI7FaAyFohPFlbiar3IQCA48Hw/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_png/udZl15qqib0Op8L2Y8lPjG250iaAE9ElgubX7T5icicqZhR8AmKib9PmjTxnqXx2NhTfZRpLHibNoRXOnlEbTBSY6EHg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9Elgueha6ETiaaZyochgr5mDnVwp2e2nkPP6uaU8d2txJJbMHas4icsMeHzbQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9ElgulmNPYe6B4ZY9D8u1C8mcFEZDjiaicY6sb0vGTmPmneicvibIz38QOz9Acw/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

我们只需要打开待测试的网页，然后点击Page Speed里的 Start analyzing按钮，它就会自动帮我们测试网络传输性能了，这是我的网站测试结果：

![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguzvW6jZvKNFjWict7Bb6LZkJL6ydIdjRmicwBUsFEzZzeppZbx3w0qh7w/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_png/udZl15qqib0Op8L2Y8lPjG250iaAE9ElgutUT0ia4rAiaYianLrkxtibnIIHibq5ebfGnBtPsaOLHpJ5OswtYQsBKBcQg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_png/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguiaSHnY1asbYbs1PAeKYbrCTG819ib75ibq4sRnKAWqoFpkrRmJCjqu0jQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_png/udZl15qqib0Op8L2Y8lPjG250iaAE9ElgujlaYQ0jQzQLploS5IbkMvMJgvQ0CgG3b20czEzVmnBWoQ7GMmAwwYg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguQ7kickL0rtqqqoYicmOy7hELAmvCLNXctS1x0vlHv2Cia9mM6eDZWHgVg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguT0ukHCHVdlON23l6DhhKbeDviaVeT8ne74KxyEGp1MTSju2MSkkKDlA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9ElgueqeFicDDjqsx4jMNxUkqVy6pN55HxNBHfNnUiah5SmgcZSfKLct5W7rg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
（三）DOM元素离线更新：当对DOM进行相关操作时，例如innerHTML、appendChild等都可以使用Document Fragment对象进行离线操作，待元素“组装”完成后再一次插入页面，或者使用`display:none` 对元素隐藏，在元素“消失”后进行相关操作。
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguKVOYedHWtCGxsMDPgxtSLULLliaNfvkEPA8iaOaA1BPZOTpbIjC1sGdw/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_png/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguPTBFRtbLgovApBWWUAyIpiaTvRCCK5Z9tZayCmmgK7LIQPGTpj1fFIA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9ElguxPd3egQmvLxQxTLv3l32Vxtr2g0yCXtXnkZd9jqab02bdQa7cHVkLg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

### 4.3.nginx搭建反向代理

在开始搭建工作之前，首先得知道什么是反向代理。可能大家对这个名词比较陌生，先上一张图：

![img](https://mmbiz.qpic.cn/mmbiz_jpg/udZl15qqib0Op8L2Y8lPjG250iaAE9Elgu34vgia86ibqL95bl1qju0XPQjYpY0LmhcuW22PiaPBYNZLgJCKyFggMAA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
